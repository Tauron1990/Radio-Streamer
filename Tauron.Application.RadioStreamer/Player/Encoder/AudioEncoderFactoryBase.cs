...
namespace Tauron.Application.RadioStreamer.Player.Encoder
{
    ...
    public abstract class AudioEncoderFactoryBase : ObservableObject, ICommonProfileProvider
    {
        ...
        {
            public abstract void Set<TType>(TType value, [NotNull] string name);
            public abstract TType Get<TType>([NotNull] string name);

            [NotNull]
            public abstract IEncoder GetAudioEncoder([NotNull] IPlayerStream channel);
        }

        [PublicAPI]
        protected sealed class GenericMapper<TEncoder> : BaseMapper
            where  TEncoder : AudioEncoder, IEncoder
            
        {
            ...
            private readonly Func<IPlayerStream, TEncoder> _factory;
            ...
            public GenericMapper([NotNull] CommonProfile profile, [NotNull] Func<IPlayerStream, TEncoder> factory,
            ...
            {
                foreach (var dataProperty in _profile.Properties)
                {
            ...
            public override void Set<TType>(TType value, string name)
            {
                var mapperProp = _dataProperties.FirstOrDefault(mp => mp.Name == name);
                if (mapperProp == null) return;
            ...
            public override IEncoder GetAudioEncoder(IPlayerStream channel)
            {
        ...
        private CommonProfile _profile;
        private BaseMapper _baseMapper;

        [NotNull]
        public CommonProfile Profile
        {
            get { return _profile ?? (_profile = new CommonProfile(_id)); }
            set
            {
                if (value != null && value.Id != _id)
        ...
        [NotNull]
        public IEncoder CreateEncoder([NotNull] IPlayerStream channel)
        {
            EnsureBaseMapper();
...
// ReSharper disable once AssignNullToNotNullAttribute
        ...
        [NotNull]
        protected GenericMapper<TEncoder> CreateMapping<TEncoder>([NotNull] Func<IPlayerStream, TEncoder> creator)
            where TEncoder : AudioEncoder, IEncoder
        {
