...
namespace Tauron.Application.RadioStreamer.Player.CoreOld
{
	...
	/// <summary>
	/// Summary description for BassStream.
	/// </summary>
	public class BassStream : AdvancedChannel
	{
		#region Enums
		...
		protected BassStream(int handle)
            : base(handle)
		{
		}
		...
		public virtual long Length 
		{
			...
			{
				if (_disposed)
					throw new ObjectDisposedException("BassStream");

				long result = Bass.BASS_ChannelGetLength(Handle);
	    ...
	    public TAG_INFO Tags
	    {
	        get
	        {
	            var temp = new TAG_INFO();
                if(!BassTags.BASS_TAG_GetFromFile(Handle, temp))
