...
namespace MediaPortal.Player
{
	...
	/// </summary>
	public class BassAudioEngine : MediaPortal.Player.IPlayer, IDisposable
	{
		#region Enums
		/// <summary>
		...
		public List<int> LoadedPluginHandles
		{
			get { return DecoderPluginHandles; }
			set { DecoderPluginHandles = value; }
		}
		...
		private int _CrossFadeIntervalMS = 1000;
		private int _DefaultCrossFadeIntervalMS = 1000;
		...
		private void Initialize()
		{
			try
			{
				// Console.WriteLine("BASS: Initialize BASS environment ...");
				//LoadSettings();


				Banshee.Bass.Reg.Registration.Register();
				...
				//LoadDSPPlugins();
				Console.WriteLine("BASS: Initializing BASS environment done.");
			...
			{
				Console.WriteLine("BASS: Initialize thread failed.  Reason: {0}", ex.Message);
			}
		}
		...
		public void FreeBass()
		{
			if (!_BassFreed)
			{
				Console.WriteLine("BASS: Freeing BASS. Non-audio media playback requested.");
				if (_useASIO)
				{
					BassAsio.BASS_ASIO_Stop();
					BassAsio.BASS_ASIO_Free();
		...
		public void InitBass()
		{
			try
			{
				...
				if (_useASIO)
				{
					Console.WriteLine("BASS: Using ASIO device: {0}", _asioDevice);
					...
					if (_asioDeviceNumber > -1)
					{
						...
						Console.WriteLine("BASS: ASIO output to {0} channels", _asioNumberChannels);
					...
					{
						initOK = false;
						Console.WriteLine("BASS: Specified ASIO device not found. BASS is disabled.");
					}
				}
				else
				{
				...
				if (initOK)
				{
					...
					UpdateTimer.Elapsed += new System.Timers.ElapsedEventHandler(OnUpdateTimerTick);
					Console.WriteLine("BASS: Initialization done.");
					_Initialized = true;
					_BassFreed = false;
				}
				else
				{
					...
					if (_useASIO)
					{
						...
						Console.WriteLine("BASS: Error initializing BASS audio engine {0} Asio: {1}", Enum.GetName(typeof(BASSErrorCode), error), Enum.GetName(typeof(BASSErrorCode), errorasio));
					}
					else
						Console.WriteLine("BASS: Error initializing BASS audio engine {0}", Enum.GetName(typeof(BASSErrorCode), error));
				}
			}
			catch (Exception ex)
			{
				Console.WriteLine("BASS: Initialize failed. Reason: {0}", ex.Message);
			}
		}
		...
		private int GetSoundDevice()
		{
			...
			{
				Console.WriteLine("BASS: Using default Sound Device");
				sounddevice = -1;
			}
			else
			{
				...
				if (!foundDevice)
				{
					Console.WriteLine("BASS: specified Sound device does not exist. Using default Sound Device");
					sounddevice = -1;
				}
				else
				{
					Console.WriteLine("BASS: Using Sound Device {0}", _SoundDevice);
				}
			}
			return sounddevice;
		}
		...
		//    Console.WriteLine("BASS: Creating visualization...");
		...
		//        Console.WriteLine("BASS: Create visualization {0}", (result ? "succeeded" : "failed"));
		//    }

		//    catch (Exception ex)
		//    {
		//        Console.WriteLine("BASS: Error creating visualization - {0}", ex.Message);
		//        result = false;
		//    }
		...
		{
			Console.WriteLine("BASS: Loading audio decoder add-ins...");

			string appPath = System.Windows.Forms.Application.StartupPath;
			string decoderFolderPath = appPath + @"\audioplugins";

			if (!Directory.Exists(decoderFolderPath))
			{
				Console.WriteLine(@"BASS: Unable to find \musicplayer\plugins\audio decoders folder in MediaPortal.exe path.");
				return;
			}
			...
			foreach (FileInfo file in decoders)
			{
				...
				Console.WriteLine("  Core Audioplayer: Loading: {0}", file.FullName);
				...
				if (pluginHandle != 0)
				{
					...
					decoderCount++;
					Console.WriteLine("BASS: Added: {0}", file.FullName);
				}

				else
				{
					Console.WriteLine("BASS: Unable to load: {0}", file.FullName);
				}
			}

			if (decoderCount > 0)
				Console.WriteLine("BASS: Loaded {0} Audio Decoders.", decoderCount);

			else
				Console.WriteLine(@"BASS: No audio decoders were loaded. Confirm decoders are present in \musicplayer\plugins\audio decoders folder.");
		}
		...
		//{
		//    Console.WriteLine("BASS: Loading DSP plugins ...");
		//    _dspActive = false;
		//    // BASS DSP/FX
		...
		//        else
		//        {
		//            Console.WriteLine("Couldn't load VST Plugin {0}. Error code: {1}", plugin, Bass.BASS_ErrorGetCode());
		//        }
		//    }
		...
		//    Console.WriteLine("BASS: Finished loading DSP plugins ...");
		//}
		...
		public override bool Play(string filePath)
		{
			...
			try
			{
				...
				if (filePath != String.Empty)
				{
					...
					if (_useASIO || _Mixing)
						...
						streamFlags = BASSStream.BASS_SAMPLE_FLOAT;//Un4seen.Bass.BASSStream.BASS_DEFAULT;//BASSStream.BASS_STREAM_AUTOFREE;
					...
					if (IsCDDA(filePath))
					{
						...
						if (stream == 0)
							Console.WriteLine("BASS: CD: {0}.", Enum.GetName(typeof(BASSErrorCode), Bass.BASS_ErrorGetCode()));
					}
							 ...
							 filePath.ToLower().StartsWith("mms"))
					{
						...
						//Console.WriteLine("BASSAudio: Webstream found - trying to fetch stream {0}", Convert.ToString(stream));
					}
					...
					else
					{
						...
						Console.WriteLine("BASS: Unable to play {0}.  Reason: {1}.", filePath, Enum.GetName(typeof(BASSErrorCode), error));
			...
			{
				result = false;
				//Console.WriteLine("BASS: Play caused an exception:  {0}.", ex);
			}
		...
		private int RegisterPlaybackFadeOutEvent(int stream, int streamIndex, int fadeOutMS)
		{
			...
			{
				int error = Bass.BASS_ErrorGetCode();
				//Console.WriteLine("BASS: RegisterPlaybackFadeOutEvent of stream {0} failed with error {1}", stream, error);
			}
		...
		private int RegisterPlaybackEndEvent(int stream, int streamIndex)
		{
			...
			{
				int error = Bass.BASS_ErrorGetCode();
				//Console.WriteLine("BASS: RegisterPlaybackEndEvent of stream {0} failed with error {1}", stream, error);
			}
		...
		private int RegisterStreamFreedEvent(int stream)
		{
			...
			{
				int error = Bass.BASS_ErrorGetCode();
				//Console.WriteLine("BASS: RegisterStreamFreedEvent of stream {0} failed with error {1}", stream, error);
			}
		...
		{
			//Console.WriteLine("BASS: PlaybackFadeOutProc of stream {0}", stream);
			...
			if (removed)
				...
				Console.WriteLine("BassAudio: *** BASS_ChannelRemoveSync in PlaybackFadeOutProc");
		}
		...
		{
			Console.WriteLine("BASS: PlaybackEndProc of stream {0}", stream);
			...
			bool removed = Bass.BASS_ChannelRemoveSync(stream, handle);
			if (removed)
				Console.WriteLine("BassAudio: *** BASS_ChannelRemoveSync in PlaybackEndProc");
		}
		...
		private void PlaybackStreamFreedProc(int handle, int stream, int data, int userData)
		{
			...
			Console.WriteLine("BASS: PlaybackStreamFreedProc of stream {0}", stream);
		...
		//            Console.WriteLine("BASS: Connection Information: {0}", item);
		//        }
		//    }
		//}
		...
		private void GetMetaTags(IntPtr tagPtr)
		{
			...
			try
			{
				...
				if (tag != null)
				{
					...
					_prevMetaTag = tag;
					Console.WriteLine("BASS: Title sent via Stream: {0}", tag);
		...
		//            if (syncpos > -1)
		//            {
		//                Console.WriteLine("BASS: last.fm song changed");
		//                int stream = GetCurrentStream();
		...
		//    catch (Exception ex)
		//    {
		//        Console.WriteLine("BASS: Exception in Downloadproc: {0} {1}", ex.Message, ex.StackTrace);
		//    }
		//}
		...
		{
			Console.WriteLine("BASS: HandleSongEnded - manualStop: {0}, CrossFading: {1}", bManualStop, _CrossFading);
			PlayState oldState = _State;
			if (!bManualStop)
			{
				if (_CrossFading)
		...
		{
			Console.WriteLine("BASS: FadeOutStop of stream {0}", stream);
		...
		public override void Pause()
		{
			...
			Console.WriteLine("BASS: Pause of stream {0}", stream);
			try
			{
		...
		public override void Stop()
		{
			...
			int stream = GetCurrentStream();
			Console.WriteLine("BASS: Stop of stream {0}", stream);
			try
			{
			...
			{
				Console.WriteLine("BASS: Stop command caused an exception - {0}", ex.Message);
			}
		...
		{
			Console.WriteLine("BASS: SeekForward for {0} ms", Convert.ToString(ms));
			_CrossFading = false;
		...
		{
			Console.WriteLine("BASS: SeekReverse for {0} ms", Convert.ToString(ms));
			_CrossFading = false;
		...
		{
			Console.WriteLine("BASS: SeekToTimePosition: {0} ", Convert.ToString(position));
			_CrossFading = false;
